...
int main(void){
  ...
  while (1)
  {
	...
	//TO DO:
	//TASK 6
//	uint8_t val = decToBcd(500);
	//int bin = bcdToDec(decToBcd(5));
//
//	sprintf (buffer, "%d \r \n", val);
...
uint8_t decToBcd(int val)
{
	...
	//YOUR CODE HERE
//		char* j = "A"; //variable to return from the method
//	    char k[1024] = ""; //returns an errror because of local variable
//	    char rev[1024] = "";
//	    int a = 0; //counter for the array
//	    int m = decValue; // keeps track of the current decimal/number value
//
//	    //loop until the decimal value/number reaches zero
//	    while(m != 0)
//	    {
//	        int num = m/2; //get the integer value of decimal value divided by radix
//
//	        int rem = m - (2*num); //find the remainder
//
//	        char h; //holds the current remainder char
//
//	        //checks if the rem is more than 10 in order to change the char for hexadecimal [10-15 -> (A-F)]
//	        if (rem >= 0 && rem <= 9)
//	        {
//	            h =  (char)(rem + '0'); //casts remainder int to a char
//	        }
//	        else
//	        {
//	            h = (char)(rem - 10 + 'A');
//	        }
//
//	        m = num; //assigns m to the new num
//
//	        k[a] = h; //add the remainder to the char array
//	        a++; //increment the counter
//
//	    }
//
//	    j = k; //reverses the char string
